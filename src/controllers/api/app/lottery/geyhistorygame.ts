...
router.get("/tickets/:type", async (req: Request, res: Response) => {
    ...
    try {
        if (typeof req.query.id !== "undefined") {
            ...
            let condition = (req.params.type == 'win' && resultsDetail != '') ? "`LotteryTicketModel`.`userId`='"+ user.id +"' AND `LotteryTicketModel`.`orderStatus` = '"+ orderStatus +"' AND `LotteryTicketModel`.`resultDetail` = '"+ resultsDetail +"'" : "`LotteryTicketModel`.`userId`='"+ user.id +"' AND `LotteryTicketModel`.`orderStatus` = '"+ orderStatus +"'";
